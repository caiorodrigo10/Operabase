...
// Service role key (bypasses RLS) - this is typically stored in environment variables
const SUPABASE_SERVICE_KEY = process.env.SUPABASE_SERVICE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imxrd3Jldmh4dWdheGZwd2lrdGR5Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc0OTgyODQ2MywiZXhwIjoyMDY1NDA0NDYzfQ.Hzqr8mVPnJvwKGjQF-LlKzKqXlqKXxnKQJ8PZB7wqWU';

// Enhanced Supabase client that bypasses RLS
async function supabaseQuery(table, filters = {}) {
  try {
    ...
    logMessage(`🔍 Supabase query (SERVICE ROLE): ${url}`);
    ...
    logMessage(`✅ Supabase data received (SERVICE ROLE): ${data.length} records`);
    return data;
  } catch (error) {
...
app.get('/health', (req, res) => {
  const healthData = {
    status: 'ok',
    message: 'Enhanced Debug Server v2 is running!',
    ...
    features: ['appointments', 'contacts', 'calendar', 'supabase', 'service-role', 'rls-bypass']
  };
...
app.get('/api/test', (req, res) => {
  ...
  res.json({
    message: 'Enhanced Debug API v2 is working!',
    timestamp: new Date().toISOString(),
    headers: req.headers,
    query: req.query,
    features: ['appointments', 'contacts', 'calendar', 'service-role']
  });
});

// ===== ROTAS ESSENCIAIS COM RLS BYPASS =====
...
// Startup
logMessage('🚀 Enhanced Debug Server v2 iniciando...');
logMessage(`📍 PORT: ${PORT}`);
logMessage(`📍 NODE_ENV: ${process.env.NODE_ENV}`);
logMessage(`📍 Process PID: ${process.pid}`);

app.listen(PORT, () => {
  logMessage('✅ Enhanced Debug Server v2 configurado e aguardando conexões...');
  logMessage(`🚀 Enhanced Debug Server v2 rodando na porta ${PORT}`);
  logMessage(`📍 Ambiente: ${process.env.NODE_ENV || 'production'}`);
  logMessage(`📍 Commit: 420a879 - Versão funcionando + rotas essenciais + RLS bypass`);
  logMessage(`📍 Features: appointments, contacts, calendar, supabase, service-role`);

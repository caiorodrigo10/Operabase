...
function isAutoGeneratedContent(content: string): boolean {
  ...
  const autoGeneratedPatterns = [
    /^📎\s*.+\.(jpg|jpeg|png|gif|pdf|doc|docx|mp3|mp4|wav|ogg|avi|mov)$/i, // File with emoji prefix
    /^.+\.(jpg|jpeg|png|gif|pdf|doc|docx|mp3|mp4|wav|ogg|avi|mov)$/i, // Just filename
    /^(documento|imagem|áudio|vídeo|arquivo)\s*enviado/i, // System messages
    /^whatsapp-audio-\d+\.mp4$/i, // WhatsApp audio files
    /^\d{13}-.*\.(mp3|mp4|wav|ogg)$/i, // Timestamp-based filenames
    /^(audio|image|video|document)_\d+/i, // Generic file patterns
  ];
  
  return autoGeneratedPatterns.some(pattern => pattern.test(content.trim()));
}
    ...
    <div className={cn("flex mb-3", isReceived ? "justify-start" : "justify-end")}>
      ...
      <div className="flex flex-col max-w-xs lg:max-w-md">
        ...
        >
          ...
          {/* Text content - only show if it's meaningful text, not auto-generated file names */}
          {message.content && !isAutoGeneratedContent(message.content) && (
